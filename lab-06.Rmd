...
```{r}
diamonds <- filter(diamonds, length_mm > 0, width_mm > 0 )
summary(diamonds)
count(diamonds,length_mm, depth_mm, sort = TRUE)
ggplot(diamonds,mapping = aes(x = length_mm , y = depth_pct)) + geom_point()
ggplot(diamonds,mapping = aes(x = width_mm, y = depth_pct)) + geom_point()
